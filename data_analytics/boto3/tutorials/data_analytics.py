...
補足：データの変換方法
　ETL(Extract Transform Load)・・・データを抽出(Extract)して、データをツールなどで変換(transform)した後、データストアにLoadする
　ELT(Extract Load Transform)・・・ロード可能な形式でデータを抽出(Extract)して、データストアにLoadした後、SQLなどでデータを変換する
...
データレイク層(S3)・・・部門DBからデータ取得。
ステージング層・・・データレイク層のデータをロード。ELTができる準備を整わせる。
DWH層・・・ステージング層のデータを統合化。
データマート層・・・DWH層のデータを集計。データ蓄積。
...
　　　　　　　　　データをそのままの形で蓄積する。
S3・・・Amazon流ではデータレイクとして用いられる。
データカタログ・・・メタデータ(データに関する情報)の管理台帳。
AWS Glue・・・S3からデータを取得し、データカタログのJOBを実行。
...
ツール
データを持ってくる・・・alteryx(アルタリクス)
データをためる・・・Amazon Redshift, Google BigQuery, Snowflake
データを可視化する・・・tableau, DOMA, Looker
